...
async function evaluateSuite(){
 ...
 for(const size of [16,24,48]){
  ...
  for(const bg of cfg.backgrounds)for(const method of ['0.6','0.9',...cfg.methods]){
...
async function timing(){
 await atlas(48);const alphas=(await(await fetch('timing-protocol-v3.json')).json()).alphas,samples=Object.fromEntries(alphas.map(a=>[a,[]]));
...
async function nativeCheck(){
 ...
 for(const mode of ['source-over','difference','exclusion'])for(const a of [.6,.9,1]){
